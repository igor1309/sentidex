...
describe('process-messages script (Characterization Test)', () => {
  ...
  describe('Scenario 1: single file', () => {
    ...
    test('should keep #g-gemini tag when AI returns empty tags for bundle payload', async () => {
      const bundledMessage = frontMatterCodec.stringify({
        frontMatter: {
          raw_message: true,
          ...
          debug: {
            message_ids: [886, 887],
            bundle_start_at: '2026-02-21T04:49:30.000Z',
            bundle_end_at: '2026-02-21T04:49:30.000Z',
            bundle_status: 'normal',
            forwarded_messages: [
              {
                message_id: 887,
                timestamp: '2026-02-21T04:49:30.000Z',
                content: 'Gemini 3.1 Pro уже в AI Studio. Цены те же, но уверен все там поднялось по всем параметрам. Ждем анонса',
                source_info: 'AI Product | Igor Akimov',
                source_url: 'https://t.me/ai_product/1956',
                forward_date: '2026-02-19T15:56:27.000Z',
                has_media: true,
                media_type: 'photo',
                forward_protected: false,
              },
        ...
        bodyContent: [
          ...
          '==== FORWARDS ====',
          '',
          '---- Forward 1 (message_id: 887) ----',
          'Source: AI Product | Igor Akimov',
          '',
          'Gemini 3.1 Pro уже в AI Studio. Цены те же, но уверен все там поднялось по всем параметрам. Ждем анонса',
        ].join('\n'),
      });
      ...
      const outputFile = fs.readdirSync('/inbox')[0];
      const output = fs.readFileSync(path.join('/inbox', outputFile), 'utf8');
      const { frontMatter } = frontMatterCodec.parse(output);

      expect(frontMatter.tags).toEqual(['g-gemini']);
    });
  });
